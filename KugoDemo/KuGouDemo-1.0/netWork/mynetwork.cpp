...
const static QString KUGOSongSearch = "http://mobilecdn.kugou.com/api/v3/search/song?format=json&keyword=%1&page=1&pagesize=18";
/*
...
////////////////////////////////////////////////////////////////////////////////////////
void MyNetWork::reqSongInfo(const QString &hashStr, const QString& albumID)
{
	QNetworkRequest request_SongInfo;
    QNetworkAccessManager manager;
	QString KGAPISTR1 = QString("http://www.kugou.com/yy/index.php?r=play/getdata"
		"&hash=%1&album_id=%2&_=1497972864535").arg(hashStr).arg(albumID);
    request_SongInfo.setAttribute(QNetworkRequest::FollowRedirectsAttribute, true);
    request_SongInfo.setUrl(QUrl(KGAPISTR1));
	//这句话很重要，我们手动复制url放到浏览器可以获取json，但是通过代码不行，必须加上下面这句才可以
	request_SongInfo.setRawHeader("Cookie", "kg_mid=2333");
	request_SongInfo.setHeader(QNetworkRequest::CookieHeader, 2333);
    QNetworkReply* reply1 = manager.get(request_SongInfo);
	...
	QString errorstring = reply1->errorString();
	qDebug() << "errorstring:" << errorstring;
	QVariant status_code = reply1->attribute(QNetworkRequest::HttpStatusCodeAttribute);
	qDebug() << "status_code:" << status_code.toInt();
	qDebug() << "reply:" << reply1->error();


    if (reply1->error() == QNetworkReply::NoError)
	{
		QByteArray bytes = reply1->readAll();  //获取字节 ??空字节
		QString result(bytes);  //转化为字符串
        qDebug() << result;
        emit sig_reqSongInfo(bytes);
	}
	else
	{
		//处理错误
...
///////////////////////////////////////////////////////////////////////////////////////
void MyNetWork::requestSong(const QString &str)//请求网络搜索歌曲
{
    ...
    QString Url = KUGOSongSearch.arg(songname);
...
///
    ...
    QVariant status_code = reply1->attribute(QNetworkRequest::HttpStatusCodeAttribute);
    auto tmp = reply1->error();
   if(reply1->error()==QNetworkReply::NoError)
   {
        QByteArray arry=reply1->readAll();
		QString result(arry);  //转化为字符串
		qDebug() << result;
...
#else
    QString Url = KUGOSongSearch.arg(m_songname);
    requestsong.setUrl(Url);
    QNetworkReply *reply1= mangersong.get(requestsong);
#endif
///loop1
