...
void mainWindow::initNetwork()
{
	...
	connect(m_middwid->m_rightWid->m_searchwid, SIGNAL(sig_requestSongInfo(QString,QString)),m_net,SLOT(reqSongInfo(QString, QString)));
	...
	connect(m_net, SIGNAL(sig_reqSongInfo(QByteArray)), m_middwid->m_rightWid->m_searchwid, SLOT(slot_requestSongInfo(QByteArray)));
...
void mainWindow::initConnection()
{
	...
	connect(m_ffplayer, SIGNAL(sig_CurrentMediaChange(QString)), this, SLOT(slot_currentMediaChanged(QString)));
...
void mainWindow::slot_currentMediaChanged(const QString& media)//setnowplaytext
{
	setOriginalStatus();
