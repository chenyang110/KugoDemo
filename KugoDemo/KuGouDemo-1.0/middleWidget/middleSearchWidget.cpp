...
void middleSearchWidget::slot_requestSongInfo(const QByteArray& byt)
{
	...
	QJsonDocument parse_doucment = QJsonDocument::fromJson(byt, &json_error);
								...
								if (value.isObject())
								{
									...
									if (object.contains("hash")) //hash
									{
										...
										if (FileHash_value.isString())
										{
											...
											//用Vector保存每首歌曲的hash
											m_Hashlist.append(FileHash_value.toString());
										}
									}
									if (object.contains("album_id"))
									{
										...
										if (FileHash_value.isString())
										{
                                            ...
                                            albumID = FileHash_value.toString();
											m_AlbumID.append(FileHash_value.toString());
										}
									}
                                    ...
                                    //m_songlist << object.value("songname_original").toString();//添加mp3Url??
									//将解析出的内容放到列表中
                                    m_table->insertRow(i);
									m_table->setItem(i, 0, new QTableWidgetItem(""));
                                    m_table->setItem(i, 1, new QTableWidgetItem(songname_original));
									//文字居中
                                    m_table->setItem(i, 2, new QTableWidgetItem(singername));
                                    m_table->setItem(i, 3, new QTableWidgetItem(album_name));
									QString time = QString("%1:%2").arg(duration / 60).arg(duration % 60);
                                    m_table->setItem(i, 4, new QTableWidgetItem(time));
                                    m_table->setItem(i, 5, new QTableWidgetItem(""));
								}
                                emit sig_requestSongInfo(hashStr, albumID);
							}
						}
					}
				}
